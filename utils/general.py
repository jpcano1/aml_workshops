...
def read_listdir(dir_: str) -> List[str]:
    """
...
def create_and_verify(*args: List[str], list_: bool = False) -> List[str]:
    """
...
def extract_file(filename: str, dst: Optional[str] = None) -> None:
    """
    Function to extract files.

    :param filename: The filename of new file
    :param dst: Destination of new file extracted
    """
    flag = False
...
def visualize_subplot(
    ...
    assert len(imgs) == len(
        titles
    ), "La lista de imágenes y de títulos debe ser del mismo tamaño"
...
def scale(
    img: np.ndarray, min_: float, max_: float, dtype: str = "uint8"
) -> np.ndarray:
    img_min = img.min()
    img_max = img.max()
